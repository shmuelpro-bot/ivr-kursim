...
switch ($step) {
    ...
    // ================================================================
    //  OWNER MANAGEMENT  (update price / delete listing)
    // ================================================================
    ...
    case 'owner_check': {
        ...
        $roomsTxt = $apt['bedrooms'] == 0 ? 'סטודיו' : $apt['bedrooms'] . ' חדרי שינה';
        $count    = count($myApts);
        $countTxt = $count > 1 ? 'יש לך ' . $count . ' פרסומים פעילים. מציג את האחרון.' : '';
        ...
        respond([
            'type'            => 'menu',
            'id_list_message' => array_values($msgs),
    ...
    case 'stats': {
        ...
        // Top 3 cities
        $top = array_slice($cityCounts, 0, 3, true);
        foreach ($top as $city => $cnt) {
            $msgs[] = $city . ': ' . $cnt . ($cnt === 1 ? ' דירה' : ' דירות') . '.';
        }

        // Most popular apartment type
        if (!empty($typeCounts)) {
            $topType = array_key_first($typeCounts);
            $msgs[]  = 'הסוג הנפוץ ביותר: ' . $topType . '.';
        }

        // Price range
        if (!empty($prices)) {
            $avg    = round(array_sum($prices) / count($prices));
            $msgs[] = 'מחיר ממוצע ללילה: ' . $avg . ' שקל.';
