...
add_action( 'manage_maagarim_apt_posts_custom_column', function ( string $col, int $post_id ) {
    switch ( $col ) {
        case 'mg_city':
            ...
            $display = $city_id > 0
                ? ( MG_CITIES[ $city_id ] ?? $city_id )
                : ( get_post_meta( $post_id, '_maagarim_city_name', true ) ?: '—' );
...
function mg_api_handler( WP_REST_Request $request ): WP_REST_Response {
    ...
    switch ( $action ) {
        ...
        case 'all_wanted':
            return mg_action_all_wanted();

        case 'my_wanted':
            return mg_action_my_wanted( $token );

        case 'publish_wanted':
            return mg_action_publish_wanted( $body, $token );
...
function mg_action_all_listings( array $body ): WP_REST_Response {
    // Always exclude wanted listings
    $meta_query = [
        'relation' => 'AND',
        [
            'relation' => 'OR',
            [ 'key' => '_maagarim_listing_type', 'value' => 'apt' ],
            [ 'key' => '_maagarim_listing_type', 'compare' => 'NOT EXISTS' ],
        ],
    ];
...
function mg_action_all_wanted(): WP_REST_Response {
    $posts    = get_posts( [
        'post_type'      => 'maagarim_apt',
        'post_status'    => 'publish',
        'posts_per_page' => 100,
        'orderby'        => 'date',
        'order'          => 'DESC',
        'meta_query'     => [ [ 'key' => '_maagarim_listing_type', 'value' => 'wanted' ] ],
    ] );
    $listings = array_map( fn( $p ) => mg_post_to_listing( $p->ID ), $posts );
...
function mg_action_my_wanted( string $token ): WP_REST_Response {
    $email = mg_email_from_token( $token );
    ...
    $posts    = get_posts( [
        ...
        'meta_query'     => [
            'relation' => 'AND',
            [ 'key' => '_maagarim_email',        'value' => $email   ],
            [ 'key' => '_maagarim_listing_type', 'value' => 'wanted' ],
        ],
    ] );
    $listings = array_map( fn( $p ) => mg_post_to_listing( $p->ID ), $posts );
...
function mg_post_to_listing( int $post_id ): array {
    ...
    $city_id           = (int) $get( '_maagarim_city', 0 );
    $city_name_display = $city_id > 0
        ? ( MG_CITIES[ $city_id ] ?? $get( '_maagarim_city_name', '' ) )
        : $get( '_maagarim_city_name', '' );

    $nh_id           = (int) $get( '_maagarim_neighborhood', 0 );
    $nh_name_display = $nh_id > 0
        ? ( MG_NEIGHBORHOODS[ $city_id ][ $nh_id ] ?? $get( '_maagarim_neighborhood_name', '' ) )
        : $get( '_maagarim_neighborhood_name', '' );

    return [
        'id'               => (string) $post_id,
        'listing_type'     => $get( '_maagarim_listing_type', 'apt' ),
        'city'             => $city_id,
        'city_name'        => $city_name_display,
        'neighborhood'     => $nh_id,
        'neighborhood_name'=> $nh_name_display,
        'street'           => $get( '_maagarim_street', '' ),
        'building_num'     => $get( '_maagarim_building_num', '' ),
...
function mg_action_publish( array $body, string $token ): WP_REST_Response {
    ...
    $title        = $display_city . ' – ' . ( MG_APT_TYPES[ $apt_type ] ?? '' );
...
// ── Action: publish_wanted ────────────────────────────────────────────────────

function mg_action_publish_wanted( array $body, string $token ): WP_REST_Response {
    $email = mg_email_from_token( $token );
    if ( ! $email ) return mg_err( 'טוקן לא מאומת – נא להתחבר מחדש', 401 );
    ...
    $post_id = wp_insert_post( [
        'post_type'   => 'maagarim_apt',
        'post_title'  => 'מחפש: ' . $display_city,
        'post_status' => 'publish',
    ], true );

    if ( is_wp_error( $post_id ) ) return mg_err( 'שגיאה ביצירת הפרסום' );

    foreach ( [
        '_maagarim_listing_type'      => 'wanted',
        '_maagarim_email'             => $email,
        '_maagarim_city'              => $city,
        '_maagarim_city_name'         => $city === 0 ? $city_name : '',
        '_maagarim_neighborhood'      => $neighborhood,
